...
import {getProductsUser} from '../../Containers/Product/GetAllUserSlice'
...
useEffect(()=>{
dispatch(getProductsUser()).then((res) => {
if (res) {
  let  p = res.payload.filter((p)=>(p.brand.slug==id.slug))
   
  let sortedProducts=[]
  setProducts(p)
  setPn(p.filter((p)=>(p.price>value[0] &&p.price<value[1])).length)
  if(Sort=="hTl")
  {
var array1 = p.slice();
...
setPro(sortedProducts.slice(fpage,lpage))
  ...
  {
    setPro(p.filter((p)=>(p.price>value[0] &&p.price<value[1])).slice(fpage,lpage))
    setFilter(p)
