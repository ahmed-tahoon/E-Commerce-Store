...
const EditProductMerchant = (props) => {
  let Merchant;
  ...
  const [merchantId, setMerchantId] = useState(merchant2.id);
  ...
  const Edit = async () => {
    ...
    try {
      axios.defaults.headers.common = {
        Authorization: merchant2.token,
      };
      await axios.put(
        "http://localhost:5000/api/product/merchant/" + BrandData.id,
        BrandData.formData
      );
      ...
      <div className="d-flex flex-column py-3 align-items-center">
        <div className="d-flex flex-column ml-3 w-50">
          ...
          <div>
            ...
            <hr />
            <button onClick={Edit} class="btn btn-primary px-4 me-2 mb-5">
              Save
            </button>
            <button
              type="button"
              ...
              class="btn btn-danger px-4 me-2 mb-5"
            >
              Delete
            </button>
...
export default EditProductMerchant;
